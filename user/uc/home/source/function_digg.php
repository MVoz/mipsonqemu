...
function gettwodigg($diggid)
{
	...
	$ret=array();
	//get prev digg
	$q=$_SGLOBAL['db']->query("SELECT diggid,url,subject FROM ".tname('digg')." WHERE diggid<".$diggid." ORDER BY diggid DESC limit 1");
	while($v=$_SGLOBAL['db']->fetch_array($q))
	{
		$prevdigg=$v;
	}
	//get next digg
	$q=$_SGLOBAL['db']->query("SELECT diggid,url,subject FROM ".tname('digg')." WHERE diggid>".$diggid." ORDER BY diggid ASC limit 1");
	while($v=$_SGLOBAL['db']->fetch_array($q))
	{
		$nextdigg=$v;
	}
