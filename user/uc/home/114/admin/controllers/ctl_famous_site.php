...
class ctl_famous_site
{
    ...
    public static function search()
    {
        try
        {
            ...
            $result = mod_famous_site::search($keyword, $search_type, $start, PAGE_ROWS);
            if (empty($result))
            {
            ...
            else
            {
                ...
                app_tpl::assign('page_url', "?c=famous_site&a=search&search_type={$search_type}&keyword={$keyword}");
    ...
    public static function index()
    {
        try
        {
            app_tpl::assign( 'npa', array('网址管理', '名站列表') );
            $show_type = 'default';
            // 按分类查看
            ...
            // 结果
            $result = mod_famous_site::get_list($class_id, $isend, $start, PAGE_ROWS);
            if (!empty($result))
            {
                app_tpl::assign('page_url', "?c=famous_site&classid={$class_id}&isend={$isend}");
                app_tpl::assign('pages', mod_pager::get_page_number_list($result['total'], $start, PAGE_ROWS));
                app_tpl::assign('list', $result['data']);
    ...
    public static function edit()
    {
        try
        {
            ...
            if ($action == 'modify')
            {
                ...
                $result = mod_famous_site::get_one($id);
                if (empty($result))
                {
            ...
            {
                app_tpl::assign( 'npa', array('网址管理', '新增名站') );
        ...
        app_tpl::display('famous_site_edit.tpl');
    }
    ...
    public static function save()
    {
        try
        {
            ...
            if ($action == 'add')
            {
                ...
                if(empty($class_list))
                {
                    throw new Exception('请先添加名站分类', 10);
                }
                $tmp = app_db::select('ylmf_mingzhan', 'id', "(name = '{$site_name}' OR url = '{$site_url}') AND class = {$class_id}");
                if (!empty($tmp))
                {
                    throw new Exception('该网站已存在');
                }

                if (false === app_db::insert('ylmf_mingzhan', array_keys($data), array_values($data)))
                {
                ...
                else
                {
                    ...
                    mod_login::message('添加成功', '?c=famous_site&classid=' . $class_id);
                }
            ...
            {
                // 检查是否已存在
                $tmp = app_db::select('ylmf_mingzhan', 'id', "id != {$id} AND (name = '{$site_name}' OR url = '{$site_url}') AND class = {$class_id}");
                if (!empty($tmp))
                {
                    throw new Exception('该网站已存在');
                }

                if ($id < 1 || false === app_db::update('ylmf_mingzhan', $data, "id = {$id}"))
                {
                    throw new Exception('数据库操作失败', 10);
                }
                else
                {
                    ...
                    mod_login::message('修改成功', '?c=famous_site&classid=' . $class_id);
                }
            }
        }
        catch (Exception $e)
    ...
    public static function list_edit()
    {
        try
        {
            $referer = (empty($_POST['referer'])) ? '?c=famous_site' : $_POST['referer'];
            // 排序
            if (!empty($_POST['order']))
            {
                foreach ($_POST['order'] as $key => $val)
                {
                    ...
                    app_db::update('ylmf_mingzhan', array('displayorder' => $val), "id = {$key}");
                }
            }
            ...
            if (!empty($_POST['recommend']))
            {
                ...
                if (!empty($condition))
                {
                    app_db::update('ylmf_mingzhan', array('good' => 1), "id IN ($condition)");
                }
            }
            if (!empty($_POST['norecommend']))
            {
                ...
                if (!empty($condition))
                {
                    app_db::update('ylmf_mingzhan', array('good' => 0), "id IN ($condition)");
                }
            }
            ...
            if (!empty($_POST['delete']))
            {
                if (empty($_POST['step']))
                {
                    ...
                    foreach ($_POST['delete'] as $key => $val)
                    {
                        $key = (int)$key;
                        $delete[] = $key;
                        $tmp = mod_fammous_site::get_one($key);
                        $name .= $tmp['name'] . ', ';
                    }
                    ...
                    app_tpl::assign('referer', $referer);
                    app_tpl::assign('action_url', '?c=famous_site&a=list_edit');
                ...
                else
                {
                    ...
                    if (!empty($condition))
                    {
                        app_db::query("INSERT INTO ylmf_recycler (table_name, sitename,siteurl,oldclass, adduser, namecolor,displayorder,remark)
                        			SELECT 'ylmf_mingzhan', name,url,class, '" . mod_login::get_username() . "', namecolor,displayorder,remark FROM ylmf_mingzhan WHERE id IN ($condition)");
                        app_db::delete('ylmf_mingzhan', "id IN ($condition)");
                    }
                }
            }
            //mod_make_html::auto_update('index');
