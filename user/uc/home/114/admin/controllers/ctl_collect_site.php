 ...
 * @version    $Id: ctl_site_manage.php 1541 2009-12-11 07:54:41Z syh $
 */
...
class ctl_collect_site
{
    ...
    public static function search()
    {
        try
        {
            ...
            $result = mod_collect_site::search($keyword, $search_type, $start, PAGE_ROWS);
            if (empty($result))
            {
            ...
            else
            {
                ...
                app_tpl::assign('page_url', "?c=collect_site&a=search&search_type={$search_type}&keyword={$keyword}");
        ...
        app_tpl::display('collect_site_list.tpl');
    ...
    public static function index()
    {
        try
        {
            ...
            // 结果
            $start = (empty($_GET['start'])) ? 0 : (int)$_GET['start'];
            $result = mod_collect_site::get_list($class_id, $isend, $start, PAGE_ROWS);

            if (!empty($result))
            {
                app_tpl::assign('list', $result['data']);
				app_tpl::assign('start', $start);
                app_tpl::assign('page_url', "?c=collect_site&classid={$class_id}&isend={$isend}");
                app_tpl::assign('pages', mod_pager::get_page_number_list($result['total'], $start, PAGE_ROWS));
            }
        ...
        try
        {
            ...
            if ($action == 'add')
            {
                ...
                if (app_db::insert('ylmf_site', array_keys($data), array_values($data)))
                {
					...
					$tagarr = mod_collect_site::batch_tag($siteid,$site_tag);
                    ...
                    mod_login::message('添加成功', '?c=collect_site&classid=' . $class_id);
                }
                else
                {
                    throw new Exception('数据库操作失败', 10);
            ...
            {
				$tagarr = mod_collect_site::batch_tag($id,$site_tag);
				$data['tag'] = empty($tagarr)?'':addslashes(serialize($tagarr));
		        $old = mod_collect_site::get_one($id);
                ...
                else
                {
                    ...
                    mod_login::message('修改成功', '?c=collect_site&classid=' . $class_id);
                }
            }
        }
        catch (Exception $e)
    ...
    public static function edit()
    {
        try
        {
            ...
            if ($action == 'add')
            {
                ...
                $result = mod_collect_site::get_one($id);
	
                if (empty($result))
                {
                    throw new Exception('没有找到数据', 10);
                }
				$result['name'] = $result['link_subject'];
				$result['tag'] = $result['link_tag'];
				$result['remark'] = $result['link_description'];
				//修正tag显示
    ...
    public static function list_edit()
    {
        try
        {
            $referer = (empty($_POST['referer'])) ? '?c=collect_site' : $_POST['referer'];
            // 排序
            ...
            if (!empty($_POST['delete']))
            {
                if (empty($_POST['step']))
                {
                    ...
                    foreach ($_POST['delete'] as $key => $val)
                    {
                        $key = (int)$key;
                        $delete[] = $key;
                        $tmp = mod_collect_site::get_one($key);
                        $name .= $tmp['name'] . ', ';
                    }
                    ...
                    app_tpl::assign('referer', $referer);
                    app_tpl::assign('action_url', '?c=collect_site&a=list_edit');
    ...
    public static function multi_add()
    {
        ...
        if(!empty($_POST))
        {
            mod_collect_site::multi_add($_POST);
            mod_login::message('批量添加成功', '?c=collect_site&classid=' . $_POST['classid']);
            exit;
        }
    ...
    public static function import()
    {
        ...
        {
            mod_collect_site::import($_POST);
            mod_login::message('网址导入成功', '?c=collect_site&classid=' . $_POST['classid']);
            exit;
        }
        $class_list = mod_class::get_subclass_list(0);
        if (empty($class_list))
