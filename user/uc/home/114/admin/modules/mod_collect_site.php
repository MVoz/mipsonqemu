 ...
 */
	 ...
	 */
	public static function collect_site_add($data)
	{
//		$data=array(
//			'name'=>'wushc测试',
//			'namecolor'=>'232323',
...
//		);
 ...
 '{$data['namecolor']}','{$data['displayorder']}','{$data['starttime']}','{$data['endtime']}','{$data['remark']}')");
	 ...
	 * @param <array> $data
	 */
	public static function collect_site_delete($delid)
	{
		if(!is_array($delid))
		{
	 ...
	 * @param <array> $data
	 */
	public static function collect_site_save($data,$type='select')
	{
//		$data=array(
//			'step'=>'2',
//			'id'=>'2',
...
//		);
	 ...
	 * @param <type> $orderby
	 */
	public static function collect_site_order($orderby)
        {
            if (is_array($orderby))
            {
	...
	{
		global  $db;
		$data=app_db::select("ylmf_siteclass" , "*","1");
		foreach ($data as $class)
		{
			app_db::query("SELECT count(*) as sum FROM ylmf_site WHERE class='$class[classid]'");
			$rt=app_db::fetch_one();
			app_db::query("UPDATE ylmf_siteclass SET sitenum='$rt[sum]'  WHERE classid='$class[classid]'");
		}
	}
