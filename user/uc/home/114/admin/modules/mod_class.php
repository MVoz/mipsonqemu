 ...
 * @version    $Id: mod_class.php 1541 2009-12-11 07:54:41Z syh $
 */
 ...
 */
    public static function update_cache_main_class($data = array())
    {
        ...
        if(!$data)
        {
            $data = app_db::select('ylmf_siteclass', '*', '1 ORDER BY displayorder');
        ...
        if($update_parent_sitenum)
        {
            foreach($class_list as $class)
            {
                ...
                app_db::query("update ylmf_siteclass set sitenum='" . $count[0]['count'] . "' where classid=$id");
    ...
    public static function update_site_count( $id_in = null )
    {
        ...
        else
        {
            ...
            app_db::query("update ylmf_siteclass set sitenum='" . $count[0]['count'] . "' where classid=$id_in");
    ...
    public static function add_class( $data = array() )
    {
        ...
        else
        {
            ...
            app_db::query("SELECT count(*) as cnt FROM ylmf_siteclass where (parentid = '$classid' AND classname = '$classnewname') $conditions");
            $rs = app_db::fetch_one();
            if($rs['cnt']>0)
            {
            ...
            else
            {
                ...
                app_db::query("INSERT INTO ylmf_siteclass (parentid,classname,displayorder,path,keywords,description)VALUES ('$classid','$classnewname','$orderid','$path','$keywords','$description')");
    ...
    public static function edit_class($data)
    {
        ...
        else
        {
            ...
            app_db::query("UPDATE ylmf_siteclass SET classname='$classnewname' ,parentid='$classid',path='$path', keywords='$keywords',description='$description'  WHERE classid='$id'");
    ...
    public static function search_class( $keyword = '' )
    {
        ...
        $search_rs = app_db::select('ylmf_siteclass', 'classid', "classname like '%" . $keyword . "%' order by parentid");
        if( !$search_rs )
        {
            return array();
        }
    ...
    public static function class_path( $data )
    {
        ...
        foreach($path as $key => $val)
        {
            $key=intval($key);
            $newpath=$val;
            app_db::query("UPDATE ylmf_siteclass SET path='$newpath' WHERE classid=$key");
            $class_list = self::get_class_list();
    ...
    public static function order_class($data )
    {
        ...
        foreach($orderid as $key => $val)
        {
            ...
            app_db::query("UPDATE ylmf_siteclass SET displayorder='$order[$key]' WHERE classid=$val");
            $class_list = self::get_class_list();
    ...
    public static function delete_class( $id )
    {
        ...
        if( $id > 0 && $subclass_list = self::get_subclass_list($id) )
        {
            ...
            app_db::delete('ylmf_siteclass', "classid = {$id}");
            $class_list = self::get_class_list();
            if($class_list[$id]['parentid'])
            {
                $class_list[$class_list[$id]['parentid']]['sub_classnum']--;
