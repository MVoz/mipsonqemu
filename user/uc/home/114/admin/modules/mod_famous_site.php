...
class mod_famous_site
{
	...
	public static function get_one($id)
	{
		...
		$data = app_db::select('ylmf_mingzhan', '*', "id = {$id}");
	...
	public static function check_exists($class_id, $site, $id = 0)
	{
		...
		if ($id > 0)
		{
		    $data = app_db::select('ylmf_mingzhan', 'id', "`class` = {$class_id} AND (`name` = '{$site}' OR url = '{$site}') AND id != {$id}");
		}
		else
		{
		    $data = app_db::select('ylmf_mingzhan', 'id', "`class` = {$class_id} AND (`name` = '{$site}' OR url = '{$site}')");
		}
	...
	public static function delete_by_class($class_id)
	{
		...
		return app_db::delete('ylmf_mingzhan', "class = {$class_id}");
	}
