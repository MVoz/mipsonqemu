 ...
 * @version    $Id: mod_trade_site.php 1541 2009-12-11 07:54:41Z syh $
 */
!defined('PATH_ADMIN') &&exit('Forbidden');
class mod_trade_site
{
	/**
	 * 返回搜索结果
	 *
	...
	public static function search($keyword, $search_type, $start = 0, $num = 0)
	{
	    ...
	    $sql = 'SELECT SQL_CALC_FOUND_ROWS a.id, b.classid AS class_id, b.classname AS class_name, a.name, a.url, a.displayorder, a.starttime, a.endtime, a.namecolor, a.adduser, a.good
                FROM ylmf_tradesite AS a, ylmf_tradeclass AS b
		...
		/*
		$data = app_db::select('ylmf_tradesite', 'SQL_CALC_FOUND_ROWS *', $contition);
		if (empty($data))
		{
			return false;
		}*/
	...
	public static function get_list($class_id = 0, $isend = false, $start = 0, $num = 20)
	{
		...
		if ($isend)
		{
			$condition .= ' AND `endtime` > 0 AND `endtime` < ' . time();
		}
	...
	public static function get_one($id)
	{
		...
		$data = app_db::select('ylmf_tradesite', '*', "id = {$id}");
	...
	public static function check_exists($class_id, $site)
	{
		...
		$data = app_db::select('ylmf_tradesite', 'id', "`class` = {$class_id} AND (`name` = '{$site}' OR url = '{$site}')");
	...
	public static function delete_by_class($class_id)
	{
		...
		return app_db::delete('ylmf_tradesite', "class = {$class_id}");
	}
}
?>
