...
class mod_site_report
{
	...
	public static function get_list($type, $start = 0, $num = 0)
	{
		...
		if ($type == -1)
		{
		    $data = app_db::select('ylmf_siteerr', 'SQL_CALC_FOUND_ROWS *', "1 {$limit}");
		}
		else
		{
		    $data = app_db::select('ylmf_siteerr', 'SQL_CALC_FOUND_ROWS *', "`type` = {$type}{$limit}");
		}
		if (empty($data))
		{
			return false;
		...
		{
			$tmp= app_db::select('ylmf_site', '*', "id = {$row['siteid']}");
		    $row = array_merge($row, $tmp[0]);
		}
	...
	public static function get_one($id)
	{
		...
		$data = app_db::select('ylmf_siteerr', '*', "id = {$id}");
		if (empty($data))
		{
		    return false;
		}
	...
	public static function get_total($type = 0)
	{
        ...
        return app_db::get_rows_num('ylmf_siteerr',  "`type` = {$type}");
	}
