<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_import|template/default/header|template/default/footer', '1266328642', 'template/default/cp_import');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<caption>
<h2><?php if($incount) { ?>导入结果<?php } else { ?>选择要导入的日志<?php } ?></h2>
<?php if($paycredit) { ?>
<p>使用本功能需要支付积分: <?=$paycredit?> (您现在的积分数 <a href="cp.php?ac=credit"><?=$space['credit']?></a>)</p>
<?php } ?>
</caption>

<?php if(is_array($results)) { foreach($results as $key => $value) { ?>
<tr>
<td><?php if(!$incount) { ?><input type="checkbox" name="ids[]" value="<?=$key?>"><?php } ?> <?php if($value['blogid']) { ?><a href="space.php?uid=<?=$space['uid']?>&do=blog&id=<?=$value['blogid']?>" target="_blank"><?=$value['title']?></a><?php } else { ?><?=$value['title']?><?php } ?></td>
<td><?=$value['dateCreated']?></td>
<?php if($incount) { ?><td><?=$value['status']?></td><?php } ?>
</tr>
<?php } } ?>
...
<tr><th>单次获取日志数</th><td><?php echo $_SCONFIG['importnum']; ?></td></tr>
