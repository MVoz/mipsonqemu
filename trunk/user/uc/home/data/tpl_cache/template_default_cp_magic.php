<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_magic|template/default/header|template/default/footer', '1266338096', 'template/default/cp_magic');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<p>购买数量: <input type="text" name="buynum" value="1" style="width:40px;" /> 个（当前最多可购买 <?=$magicstore['maxbuy']?> 个）</p>
<?php if($coupon['count']) { ?>
<p>用代金券: <input type="text" name="coupon" value="0" style="width:40px;" /> 张（每张抵用 100 积分，拥有 <?=$coupon['count']?> 张）</p>
<?php } ?>
<p class="btn_line">
<input type="hidden" name="formhash" value="<?php echo formhash() ?>" />
