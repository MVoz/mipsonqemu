 ...
 * @version    $Id: mod_make_html.php 1541 2009-12-11 07:54:41Z syh $
 */
...
class mod_make_html
{
    ...
    public static function make_html_all_catalog()
    {
        try
        {
            ...
            $rt = app_db::select('ylmf_siteclass', 'classid, classname', 'parentid = 0');
            if (empty($rt))
            {
            ...
            $rs = app_db::select('ylmf_siteclass', 'classid, classname', "parentid IN ({$pid}) ORDER BY parentid");
            if (empty($rs))
            {
        ...
        if(!empty($subclass_list))
        {
            foreach($subclass_list as $info)
            {
                ...
                else
                {
                    ...
                    $class4_query = app_db::query("SELECT classid, classname FROM ylmf_siteclass WHERE parentid = '{$info['classid']}' ORDER BY displayorder");
                    $class4_list = array();
    ...
    private static function get_html_4catalog($cid)
    {
        ...
        app_tpl::assign('URL', URL, $path_tpls_main);
        $class_list = mod_class::get_class_list();
        if ($cid < 0 || empty($class_list[$cid]) || app_db::get_rows_num('ylmf_siteclass', "parentid = '{$cid}'") < 1)
        {
            return false;
        }
        ...
        $query = app_db::query('SELECT * FROM ylmf_siteclass WHERE parentid = ' . $cid . ' ORDER BY displayorder');
        while ($info = app_db::fetch_one($query))
        {
