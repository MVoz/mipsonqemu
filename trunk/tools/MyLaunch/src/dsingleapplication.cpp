...
DTalker::DTalker(const QString & id, QObject * parent):app_id(id), QTcpServer(parent)
{
...
DListner::DListner(const QString & id, int socketDescriptor, QObject * parent):socketDescriptor(socketDescriptor), app_id(id), QThread(parent)
{
	blockSize = 0;
}
