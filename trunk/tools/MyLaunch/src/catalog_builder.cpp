		...
		switch(browserid)
			{
				case BROWSE_TYPE_NETBOOKMARK:
					break;
				case BROWSE_TYPE_IE:
					tz::readDirectory(tz::getUserFullpath(NULL,LOCAL_FULLPATH_IEFAV), &bc, 0,BROWSE_TYPE_IE,0);
				break;
	...
	while(!browserInfo[i].name.isEmpty())
	{
		...
		if( browserenable[i])
		{
			...
			case BROWSE_TYPE_NETBOOKMARK:
				...
				break;
			case BROWSE_TYPE_IE:						
				if(tz::readDirectory(tz::getUserFullpath(NULL,LOCAL_FULLPATH_IEFAV), &current_bc[BROWSE_TYPE_IE], 0,BROWSE_TYPE_IE,0))
					setBrowserInfoOpFlag(browserid, BROWSERINFO_OP_LOCAL);
				break;
			case BROWSE_TYPE_FIREFOX:
				if(!tz::checkFirefoxDir(ff_path))
