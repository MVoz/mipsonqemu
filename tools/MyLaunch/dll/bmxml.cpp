...
void bmXml::getCatalog(QList<CatItem>* items)
{
...
void bmXml::getBookmarkCatalog(QList<CatItem>* items)
{
	CatItem item;
...
void bmXml::importItem(struct bookmark_catagory *bc,int item)
{
	while (!atEnd())
	{
		readNext();
...
void bmXml::CreateCatagory(int level, QList < bookmark_catagory > *list, uint groupId, uint parentId)
{
...
void bmXml::CreateItem(int level, QList < bookmark_catagory > *list, uint parentId)
{
	struct bookmark_catagory bc;
	bc.level = level;
...
void bmXml::readBookmarkElement()
{
	//      LOG_RUN_LINE;
	while (!atEnd())
	{
...
void bmXml::bmItemToFile(QTextStream * os,bookmark_catagory& bm)
{
	uint i = 0;
	QVariant t;
...
void bmXml::bmListToXml(int flag, QList < bookmark_catagory > *list, QTextStream * os,int browserType,
...
void bmXml::dumpBookmarkList(int level, QList < bookmark_catagory > list)
{
	foreach(bookmark_catagory bm, list)
	{
		QString str;
...
#endif
void bmXml::buildLocalBmSetting(int level, QString path, QList < bookmark_catagory > list, QTextStream * os)
{
	foreach(bookmark_catagory bm, list)
	{
...
void bmXml::importCatItem(CatItem *item,int im)
{
	while (!atEnd())
	{
		readNext();
...
#endif
void bmXml::readCategoryElement()
{

}
void bmXml::readItemElement()
{
}
...
QString bmXml::productExcludeIdStr(QSqlDatabase *db)
{	
...
void bmXml::handler_line(QString line,int type)
{
	int i=0;
...
void bmXml::outToFile(QTextStream& os)
{
...
void bmXml::item_end(QTextStream& os,int type,int& finish)
{
